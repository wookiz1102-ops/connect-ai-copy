# 🧠 Wookiz World — Brain-GitHub 동기화 아키텍처 레퍼런스
...
[extension.ts] HTTP 서버 (Port 4825)
    ...
    │
    ├─ 2. 파일 저장: {brainDir}/00_Raw/2026-04-26/유튜브_성장_전략.md
    │
...
# → wookiz-world-{version}.vsix

# GitHub 릴리즈
gh release create v{version} wookiz-world-{version}.vsix -t "Release v{version}"
...
| 연동 | 주소 | 역할 |
|---------|------|------|
| Brain Inject API | `http://127.0.0.1:4825/api/brain-inject` | 외부(웹 등)에서 지식을 로컬 두뇌로 주입하는 로컬 HTTP 엔드포인트 |
| memory repo | `https://github.com/wookiz1102-ops/memory` | 지식 저장소(두뇌 백업) 예시 |

웹 페이지에서 `fetch('http://127.0.0.1:4825/api/brain-inject', { title, markdown })` 형태로
POST 요청을 보내면 이 익스텐션이 받아 두뇌 폴더에 저장 + GitHub 자동 백업한다.
